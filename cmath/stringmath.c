...
{ 
	...
	// otherwise the caller's denominator would be destroyed
	//*mirror = '\0';
	memset(mirror, 0, divisors + denom);
	strcpy(mirror, a); 
	// but the mirror can't be modified with bad values so we have a temporary copy of it too
	*tempmir = '\0';
