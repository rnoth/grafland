...
{ 
	...
	# accordingly to the backup file's triplet
	case "$SUFX" in
		gz)
			COMPRESSION="gzip" 
			;;
...
help()
{ 
	printf "\`remaster': Backup and/or remaster a linux OS\n"
	...
	printf "  remaster --backup\n\n"
        printf "Restore from an existing backup:\n"
        printf "  remaster --file=FILE --repath=/path\n\n"
        printf "Remaster \"/\" onto a bootable disk image\n"
        printf "  remaster --backup --remaster\n\n"
	exit 0
} 
